...
document.addEventListener('DOMContentLoaded', () => {
    ...
    //boton para cambiar el color de la página
    const btnCambiarEstadio = document.getElementById('btnCambiarEstadio');
    const seccionPrincipal = document.getElementById('mainContent');
    const footer = document.getElementById('footer');
    let estadoEstadioCambiado = false;
    // Guardar los valores originales solo una vez
    const tituloOriginal = tituloPrincipal.textContent;
    const colorOriginalSeccion = seccionPrincipal.style.backgroundColor || '';
    const colorOriginalFooter = footer.style.backgroundColor || '';
    const colorOriginalFooterText = footer.style.color || '';
    const tituloNuevo = '¡La Scalonetta Campeona del Mundo!';
    const colorNuevoSeccion = '#e0f7fa'; // celeste claro
    const colorNuevoFooter = '#1a5276';  // azul oscuro
    
    btnCambiarEstadio.addEventListener('click', () => {
        if (!estadoEstadioCambiado) {
            seccionPrincipal.style.backgroundColor = colorNuevoSeccion;
            footer.style.backgroundColor = colorNuevoFooter;
            footer.style.color = '#ffffff'; // Cambiamos el color del texto del footer a blanco
            tituloPrincipal.textContent = tituloNuevo;
            estadoEstadioCambiado = true;
        } else {
            seccionPrincipal.style.backgroundColor = colorOriginalSeccion;
            footer.style.backgroundColor = colorOriginalFooter;
            footer.style.color = colorOriginalFooterText; // Restauramos el color original del texto del footer
            tituloPrincipal.textContent = tituloOriginal;
            estadoEstadioCambiado = false;
        }
});
